## Lightning Autofill for Android (Dec 17, 2025)

Exciting news -- you can now use Lightning Autofill on your Android phone! 🥳

Here's how to install it:

1. Install [Edge Canary for Android](https://play.google.com/store/apps/details?id=com.microsoft.emmx.canary)
2. Enter `edge://flags` in the browser address bar
3. Search for "Android Extension Search" and enable it
4. Restart Edge
5. Tap Menu / Extensions / Manage Extensions
6. Search for "Lightning Autofill" and install it
7. Tap the Lightning Autofill menu and enable "Allow User Scripts"

![edge_mobile](https://github.com/user-attachments/assets/799104f9-8609-4c08-93df-db28dd6c30b7)

For now only Android is supported, but extensions may come to [Edge for iOS](https://apps.apple.com/us/app/microsoft-edge-ai-browser/id1288723196) in the future.

---

## Referral program bug (Dec 3, 2025)

It has been brought to our attention that a bug in our referral program code was responsible for removing existing loyalty discounts from some subscriptions. Check your last invoice -- if you notice that a discount you were receiving was no longer applied to the last invoice, please reach out to us via [email](mailto:support@tohodo.com) or [Discord DM](https://discord.gg/NY6xxsQBRD) and we'll get this corrected. Apologies for the inconvenience. 🙏

---

## A new Freemium is coming (Nov 20, 2025)

Now that v14.24.3 has been released for all three browsers, work on v15 can begin. But first, we wanted to share some exciting news about the Free plan. Let's cut to the chase: the current free tier is lousy. We have never been satisfied with it ourselves for three main reasons:

1. In hindsight, 10 free profile executions a day was not enough for more users than we anticipated based on our admittedly non-scientific estimates.
2. The free quota could be bypassed in a matter of seconds simply by reinstalling the extension.
3. The freemium code was overly complex and hard to maintain, leading to bugs and v14 getting rejected.

With the recent overhaul of our backend and major refactoring over the past six months to get v14 approved by the Chrome Web Store (CWS) team, we feel now is the right time to replace the current freemium model with something we wish we could have introduced from the beginning, but didn't have the time, resources, and expertise to at the time.

**Here's how it's going to work** (roughly, since we're still in the early stages):

- Starting with v15, every installation of Lightning Autofill will have a unique [UUID](https://developer.mozilla.org/docs/Web/API/Crypto/randomUUID), similar to a Windows license key.
- This key will be used to request autofill time from our web service. Think of it as competing for [time-share](https://www.ibm.com/history/time-sharing) on the mainframe in the 70s. In this case, you'd be competing against all other free tier users at the time you make the request.
- Our backend's load balancer will use a FIFO (First In, First Out) scheduling strategy when processing the queue. If you happen to be the first in line, your wait time is naturally zero. We cannot determine the wait time ahead of time, as it depends on too many factors to discuss here (e.g., CPU time ↔ monthly budget ↔ edge caching hits vs. misses ↔ number of concurrent free users requesting autofill time ↔ form complexity and number of fields ↔ delays from WAIT actions or Advanced tab).
- **Premium users with a valid subscription key will bypass this queue entirely.** There will never be a wait time for users on the Plus or Pro plan.

Again, the design can change between now and when v15 is released, depending on technical limitations, consultations, feedback, and various other factors, some of which might be unknown to us right now. Regardless of what the final design will be, you can be sure of one thing: **the new Free plan will have unlimited profile executions.** 🥳

---

## Lightning Autofill v14.24.3 released (Nov 11, 2025)
...
- In the previous version, we added a placeholder "Premium" tab that simply linked to the [Pricing page](https://lightningautofill.com/pricing/). This has been replaced with a proper integration so that users can subscribe directly from inside the extension's Options page.
...
## Lightning Autofill v14.18.5 released (Nov 6, 2025)

After six months of review hell and countless code changes to comply with stricter Google policies, Lightning Autofill v14 has FINALLY been approved for release to the Chrome Web Store! 🥳 Now we can proceed with long-planned features like the macro recorder and other goodies down the road.

For those updating from v14.5.4.1, it is the same as moving from v13 to v14 since v14.5.4.1 was v13.15.2 underneath. As with most major version releases in the past, the update process might not be smooth for some people. If you encounter any issues, these steps have worked for many users:

1. Back up your data to a CSV file.
2. Uninstall the extension.
3. If Chrome has pending updates, update and restart the browser. You can see if Chrome has pending updates by clicking menu / Help / About Google Chrome.
4. If Windows has pending updates, update Windows and restart your PC. You can see if Windows has pending updates by going to Settings / Windows Update.
5. Reinstall the extension.
6. Import your data from the CSV file in step 1.

Steps 3 and 4 are important because pending updates have caused problems in the past.

**Changelog:**
- Refactored to comply with CWS policies
...
## Read this if you use a password manager (Nov 4, 2025)

Since I keep reading about [data breaches in the news](https://www.forbes.com/sites/daveywinder/2025/10/28/gmail-passwords-confirmed-as-part-of-183-million-account-data-breach/), here's a shameless plug for [Password Slug](https://www.passwordslug.com/), one of my other extensions. In the downtime while waiting for the Chrome Web Store team to review LA v14, I finally brought Password Slug out of early access. After two years of private beta testing, I think it's ready for prime time.
...
I built Password Slug using a modern dev stack (Svelte + Vite + TypeScript) when [LastPass got hacked the first time](https://www.cybersecuritydive.com/news/lastpass-cyberattack-timeline/643958/), and it has worked surprisingly well. There's a one-time setup where you have to add the slug to the passwords for all the accounts you want to protect, and after that you're all set. I suggest giving Password Slug a try as a free and effective way to increase your password security. As with all our products, there's no tracking whatsoever -- you own your own data.

---

## Case of the missing welcome emails (Oct 14, 2025)

Just a quick heads-up on a minor hiccup we've run into. Due to the recent backend enhancements, the automated welcome emails got temporarily queued up and delayed. We're truly sorry for any confusion or frustration this caused, especially for our new subscribers who never received their activation instructions.

The good news: everything's sorted now, and those belated welcome emails should be landing in your inboxes today. If you're a recent member, keep an eye on your email (including spam/junk folder just in case). If you still don't see it in the next couple of hours, feel free to [reach out](mailto:support@tohodo.com). Thanks for your patience and understanding! 🙏

---

## Lightning Autofill v14.17.3 released (Oct 12, 2025)

We're excited to announce a big update with significant backend improvements and several frontend enhancements! This release focuses on infrastructure upgrades, better security, and a more powerful AI experience for the Helpdesk.

### Frontend improvements

- **New referral program** – We've replaced the Halving Pledge with an exciting new [referral program](https://lightningautofill.com/referrals/).
- **Enhanced expiration handling** – Profiles now expire at the end of the day (11:59:59 PM) rather than at the beginning of the day (midnight), which makes more practical sense when setting expiration dates.
- **Premium tab** – Users on the Free plan will now see a dedicated Premium tab to easily explore upgrade options.
- **Update notification** – Users will now see a notice in the Support tab prompting them to install the latest BETA version if they are still stuck on v14.5.4.1 (v13 underneath).
- **Simplified codebase** – Refactored the extension as part of our ongoing effort to meet Chrome Web Store requirements. After numerous review cycles, we're optimistic this update will finally get approved. 🤞

### Backend enhancements

- **Upgraded AI model** – @LightningAutofillPro now uses [Grok-4-Fast-Reasoning](https://x.ai/news/grok-4-fast) instead of GPT-4o-mini, delivering significantly higher quality responses.
- **Custom checkout solution** – We've integrated Stripe Checkout directly into our [Pricing page](https://lightningautofill.com/pricing/), moving away from the old Firebase site (autofill.tohodo.com) for better control and flexibility.
- **Improved authentication** – Switched email verification from magic links to OTP (one-time password) authentication for enhanced security.
- **Domain migration** – Moved to *.lightningautofill.com domains to ensure reliable service and avoid potential Cloudflare blocking issues.
- **Updated billing descriptor** – Your statement will now show "TOHODO" instead of "LIGHTNING AUTOFILL" to match our official LLC name (Stripe requirement).

---

## Referral program launching soon (Oct 6, 2025)

To coincide with the release of the next version of Lightning Autofill, we're replacing the Halving Pledge with a new [referral program](https://lightningautofill.com/referrals/). Full details coming when the update drops. The rewards? Let's just say the discounts you can unlock through referrals are significantly better than what the Halving Pledge offered -- we think you're going to be pleased. 🤭

---

## Lightning Autofill v14.14.5 released (Sep 5, 2025)

This minor release contains three main changes:

- We refactored a lot more code in our months long effort to get it approved by the Chrome Web Store team. Even the infamously difficult Mozilla team has approved v14, so we're actually not sure what the hold-up is with Google. We're not going to start on the macro recorder feature until v14 is released to the main Chrome channel.
- Updated links to the `lightningautofill.com` domain. That's right -- after 15 years, Lightning Autofill _finally_ has its own [official website](https://lightningautofill.com/)! 🥳
- Sometimes when you click on the Disconnect button to disconnect your Google account in Options / Sync, nothing happens. This issue has been fixed.

---

## Lightning Autofill v14 released for Firefox! (Aug 19, 2025)

After a lot of code refactoring to comply with stricter security measures, at long last Mozilla has approved v14 for release. 🥳 We are confident that it's only a matter of time before Google also approves v14 for the main Chrome channel.

Unfortunately, we will have to remove Firefox for Android support in the next update because many of the required APIs are still not supported in their mobile browser (e.g., sidebar), so if you install Lightning Autofill v14 on your phone, you won't be able to open the Options page.

---

## Lightning Autofill BETA v14.14.0 released (Aug 6, 2025)

We have a new beefy release for you today.

### Highlights

- Added support for autofilling Unicode escape sequences like `\u200B` ([zero width space](https://unicode-explorer.com/c/200B)).
- Added a dedicated blue bolt icon when manual mode is enabled: <img width="16" height="16" alt="icon16-manual" src="https://github.com/user-attachments/assets/99f940fa-0bc5-4074-acda-637ace2685bc">
- Added [keyboard shortcuts](https://docs.lightningautofill.com/help/hotkeys#commands) to open the Lightning Autofill Options page and toggle manual mode.
- Improved autofilling for more React autocomplete components ([example](https://ant.design/components/auto-complete)).
- Fixed issue where the bolt icon sometimes doesn't light up even when the page has form fields.

### Release status

- **Chrome:** main channel still on v13.15.2 (rollback v14.5.4.1); we'll submit v14.14.0 in 1-3 days if no major issues reported
- **Edge:** v14.11.2 is live; we expect v14.14.0 to roll out within 24 hours
- **Firefox:** still on v13.15.2; we'll submit v14.14.0 in 1-3 days if no major issues reported
...
## Lightning Autofill v14.11.2 released (Jul 29, 2025)

A minor update to prepare for the big v14 release on the Chrome and Firefox stores.
...
- **Chrome:** main channel still on v13.15.2 (rollback v14.5.4.1); we'll submit v14.11.2 in 1-3 days if no major issues reported
- **Edge:** v14.11.0 is live; we expect v14.11.2 to roll out within 24 hours
- **Firefox:** still on v13.15.2; we'll submit v14.11.2 in 1-3 days if no major issues reported
...
## Lightning Autofill v14.11.0 for Edge released (Jul 24, 2025)

We identified the likely root cause of why our Edge store content was deleted and reported it to Microsoft's development team. In the end, we had to re-enter all of our content in order to publish v14.11.0 to the Edge store. It should be approved and pushed out to all Edge browsers within 24 hours.

After this, our next step will be to publish v14.11.0 to the Chrome main channel and the Firefox store. Let's hope third time's the charm and Google/Mozilla will finally approve v14. 🤞

---

## Lightning Autofill BETA v14.11.0 released (Jul 17, 2025)

This release has a bunch of important bug fixes.

### Changelog

- Fixed regression: import can fail with large files (> 400KB)
...
- **Chrome:** main channel still on v13.15.2 (rollback v14.5.4.1); we'll submit v14.11.0 in 1-3 days if no major issues reported
- **Edge:** there's an issue with the submission form; we'll submit v14.11.0 once it's fixed (hopefully today or tomorrow)
- **Firefox:** still on v13.15.2; we'll submit v14.11.0 in 1-3 days if no major issues reported
...
## Halving Pledge update (Jul 16, 2025)

When we first introduced our [Halving Pledge](https://autofill.tohodo.com/halving-pledge.html) back in May, the progress bar was hovering around 1% -- we're happy to announce that as of right now, it has surpassed 20%! This means there's less than 5% to go before everyone starts to benefit from reduced prices. 🥳

<img width="947" height="130" alt="Halving Pledge progress bar" src="https://github.com/user-attachments/assets/5c4183f4-5be6-43f8-b4a7-5e60f637015e">

If you want to start saving money sooner, then please spread the word about Lightning Autofill. Here are some ideas to get you started:

- [Share on Facebook](https://www.facebook.com/sharer.php?u=https%3A%2F%2Fwww.tohodo.com%2Fautofill%2F) to let your friends and colleagues know how Lightning Autofill saves you time.
- [Tweet about us](https://x.com/intent/tweet?text=Why%20fill%20forms%20manually%20when%20Lightning%20Autofill%20exists%3F%20%40Lightning_AF%20%23automation%20%23chromeextension%20https%3A%2F%2Fwww.tohodo.com%2Fautofill%2F) and tag productivity influencers.
- [Post on Reddit](https://www.reddit.com/submit?title=Lightning%20Autofill&type=TEXT&url=https%3A%2F%2Fwww.tohodo.com%2Fautofill%2F) in Crowdtap, sweepstakes, automation, or browser extension communities.
- [Share on LinkedIn](https://www.linkedin.com/shareArticle/?mini=true&url=https%3A%2F%2Fwww.tohodo.com%2Fautofill%2F) with your professional network.
- [Leave a review](https://chromewebstore.google.com/detail/lightning-autofill/nlmmgnhgdeffjkdckmikfpnddkbbfkkk/reviews) about Lightning Autofill.
- Create an Instagram, TikTok, or YouTube video showing how you use Lightning Autofill.
- Write about us on your blog or newsletter.
- Mention us in forums and communities where form-filling is a pain point that can be automated away.

---

## Lightning Autofill BETA v14.10.0 released (Jul 9, 2025)

This release has a frequently requested feature: you can now assign a [keyboard shortcut](https://docs.lightningautofill.com/help/keyboard-shortcuts#commands) to execute the currently active profile:

![image](https://github.com/user-attachments/assets/4dca0da7-c3ef-4f7d-9f2a-4d73f562d950)

### Changelog

- Added keyboard shortcut to execute the active profile
- Fixed broken import when data has missing profiles

### Release status

- **Chrome:** main channel still on v13; we'll submit v14.10.0 in 1-3 days if no major issues reported
- **Edge:** submitted v14.10.0; expect it to be published within 24 hours
- **Firefox:** still on v13; we'll submit v14.10.0 in 1-3 days if no major issues reported
...
## Lightning Autofill BETA v14.9.0 released (Jul 8, 2025)

Version v14.8.0 was rejected by the Chrome Web Store (CWS), so this update contains more code refactoring to try to get into compliance. We won't start work on new features until v14 is released to the main channel in Chrome, so let's hope third time's the charm. 🙏

This release also includes an important bug fix to manual mode where dynamically added fields are not autofilled in some cases.
...
- **Chrome:** v14.8.0 rejected; we'll submit v14.9.0 in 1-3 days if no major issues reported for Chrome BETA
- **Edge:** we'll submit v14.9.0 in 1-3 days if no major issues reported for Chrome BETA
- **Firefox:** canceled v14.8.0 submission since it was rejected in CWS; we'll submit v14.9.0 in 1-3 days if no major issues reported for Chrome BETA
...
## Lightning Autofill v14.8.0 released (Jun 29, 2025)

This is a compliance update to try and get v14 approved by the Google and Mozilla extension review teams. It's holding up work on other features planned for v14, so let's hope we can overcome this hurdle sooner rather than later 😅. The good news is that v14.8.0 was approved for the Chrome BETA channel, so we'll submit it to the main stable channel tomorrow and cross our fingers 🤞. If you're using Chrome and want to experience all the latest features (like the side panel) and bug fixes, then we suggest you use the BETA version for now (see link below).

### Changelog

- Refactored code to comply with Google and Mozilla policies
- Fixed fail-safe backup sometimes not saving latest changes
- Fixed remote import "You have unsaved changes" bug

### Release status

- **Chrome:** v14.8.0 live on BETA channel; we'll submit it to the main channel tomorrow
- **Edge:** submitted v14.8.0 today (expect to have it approved within 24 hours)
- **Firefox:** submitted v14.8.0 today; we'll hope for the best and prepare for the worst 🙏
...
ATTENTION PRO USERS ON CHROME: Starting with Chrome v138, developer mode is no longer necessary in order to execute JavaScript rules. Instead, you need to grant a new permission in Details:
...
2. Click the **Details** button in the Lightning Autofill card (as a shortcut, you can open `chrome://extensions/?id=nlmmgnhgdeffjkdckmikfpnddkbbfkkk` directly)
3. Scroll down a bit and you should see the "Allow User Scripts" permission
4. Ensure this permission is enabled/granted
5. Exit out of Chrome completely (there should be no instances of chrome.exe in Task Manager)
6. Restart Chrome

We suspect soon Edge users will also have to go through this permission grant process, so keep this in mind if you're using Edge and suddenly JavaScript rules are no longer executing for you.
