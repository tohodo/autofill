...
- Fixed regression issue in Edge causing blank Options
...
**UPDATE:** After v12.0.3 was released, I was still getting reports of the Options page being blank in Edge even though it was working fine for me. After digging some more, I identified the real root cause: sadly, Edge in the stable channel (what most people download when they go to Microsoft) does NOT support the APIs required to implement JavaScript rules _unless_ you enable Developer mode:

![image](https://github.com/tohodo/autofill/assets/4110567/d2aeae2a-5a1f-44ea-93bc-ac3658d21663)

This explains why I was not able to reproduce the issue (because I'm always in Developer mode, which is required to develop and test extensions locally). This is not a good solution, so I am forced to unpublish Lightning Autofill for Edge until it supports the same MV3 APIs as Chrome without requiring users to enable Developer mode. This is not permanent -- I will publish Lightning Autofill for Edge again when the proper APIs are supported. Sorry for the troubles! 🙏
