...
## A word on data privacy (Dec 26, 2024)

We uploaded a new video to talk a little bit about data privacy in Chrome extensions:

https://youtu.be/ngcFqAaSJ2w

Stay safe, stay warm, happy holidays.

---

## Lightning Autofill v13.2.0 released (Dec 24, 2024)

This release fixes all known migration issues, and regression bugs related to variables.

**Changelog:**
- Added support for variables inside action rules
...
## Migration kinks (Dec 23, 2024)

It seems like a small percentage of people still have rules using regex that did not migrate to the new syntax. Good news -- it's an easy fix: wrap your Name or Site value in between forward slashes. For example, if the value is `a|b|c`, then change it to `/a|b|c/`. If you have a ton of rules that need this update, please follow [this tutorial](https://youtu.be/AbLarNoOQ-Y) to do a bulk edit.

There is also a known issue with some variables breaking -- we are working on a fix ASAP and hopefully will have a hotfix out by the end of today. For the time being, please temporarily use the Firefox version as I have postponed the v13.0.0 release for Firefox until these issues have been fixed. Sorry for the inconvenience.

---

## Lightning Autofill v13.0.0 released (Dec 21, 2024)
...
- `AUTOFILL` ([demo](https://youtu.be/YO2sIxa4Pes)) - execute a profile (this replaces the special `Autofill()` function from v11)
- `CLICK` ([demo](https://youtu.be/arbFQRwiuvM)) - click an element
- `DISPATCH` ([demo](https://youtu.be/A7j9zHM1tFI)) - send one or more events to an element
- `OPEN` ([demo](https://youtu.be/4zfCzprL-XY)) - open a URL in the current tab or a new tab
- `RELOAD` ([demo](https://youtu.be/NUHq7OPJDl8)) - reload the page until an element is found
- `SCRAPE` ([demo](https://youtu.be/zRln5RNf9AE)) - extract text from an element and store it in a temporary variable that can be used in subsequent rules
- `STYLE` ([demo](https://youtu.be/eUCGIJPzkLM)) - apply custom CSS to an element
- `WAIT` ([demo](https://youtu.be/GOTfMHTzOPc)) - wait for *N* number of seconds, or for an element to be loaded on the page

Please let us know on the [Discord server](https://discord.gg/NY6xxsQBRD) if there are other actions you would like to see in the future.

### Updated matching syntax

Version 13 has breaking changes to the matching syntax. However, all of your existing autofill rules should automatically migrate to the new syntax when you update or import, so you shouldn't have to do anything. In the rare event that your rules are still using the old syntax and your autofills stop working, then you can manually update the rules to the new syntax (see [FAQ #60](https://docs.lightningautofill.com/help/faq#q60)). The new syntax is as follows:
...
[Supported attributes »](https://docs.lightningautofill.com/help/form-fields#attributes)
...
![image](https://github.com/user-attachments/assets/7808261d-b4e9-4333-8ff4-a26cd9870dba)
...
![image](https://github.com/user-attachments/assets/9a4fe84e-d741-425d-9401-654b7ac243fe)
...
![image](https://github.com/user-attachments/assets/6ec7bde7-d01f-46e6-839c-fc352838b7e7)
...
## Last update before release (Dec 19, 2024)

We have a new video tutorial for you guys:

🎞️ [Using AI to write JS variables »](https://youtu.be/ORCUIekoqfU)

This is the last update before the big v13 release, so we cannot shout this loud enough:

**EXPORT YOUR DATA TO A CSV FILE TODAY!**

Version 13 contains *breaking changes*. There is migration code in place to automatically convert all of your rules to the new element matching syntax, but nothing is 100% guaranteed for everyone (Murphy's Law). To be on the safe side, you should make a complete backup of your data. Here's how:

1. Open Lightning Autofill Options.
2. Go to the Sync tab.
3. Click the Export button.
4. Save the CSV file in a safe location.

Now, in the rare event that the migration fails for you, we can show you how to manually migrate the data by doing a few simple search & replace operations in the CSV file. Even if there is no major release coming, you should always have a good backup strategy in place, especially if you're not using cloud sync.

---

## Three new tutorials (Dec 16, 2024)

We just published our latest video tutorials:

🎞️ [How to do bulk edits »](https://youtu.be/AbLarNoOQ-Y)<br>
🎞️ [Remote imports »](https://youtu.be/NyTFi6ZclK0)

### Data entry tutorial

We also want to give a shoutout to @imkoodos for starting a new YouTube series on "building a fully functional data entry bot". You can check out Part 1 here:

🎞️ [The New 2025 Data Entry Method (Part 1) »](https://youtu.be/uaYmtkVuuoo?si=IdCzDHtykj0rLlIc)

Be sure to subscribe to his channel as well -- he's got a lot of clever ideas on how to use variables and other insights. 💡

By the way, expect v13 to drop by the end of this week! 🥳
