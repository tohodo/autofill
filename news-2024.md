...
- **Added "Profile expiration" option (experimental)** - This was a special request by at least two users. Similar to cookies, you can now set your profiles to expire and auto-delete (along with their rules) by specifying `Expires: YYYY-DD-MM` anywhere in the profile name. For example, if you have a profile named "My Profile (Expires: 2024-08-01)", then it will be deleted at midnight on August 1st, 2024. This option is under the **Settings** / **Experimental** section.
- **Added "Turbo mode" option** - Starting with version 12.7.0, a mock delay of 0-10 ms was introduced between each keyboard event that is emitted to better simulate a human filling out the fields to improve reliability for some forms (this is different from the Delay option, which adds a delay _before_ Lightning Autofill kicks in). If you want to return to the previous behavior without this artificial delay, then you can enable turbo mode.
