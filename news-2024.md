...
- **Added "Turbo mode" option** - Starting with version 12.7.0, a mock delay of 0-10 ms was introduced between each autofill operation to better simulate a human filling out the fields to improve reliability for some forms (this is different from the Delay option, which adds a delay _before_ Lightning Autofill kicks in). If you want to return to the previous behavior without this artificial delay, then you can enable turbo mode.
